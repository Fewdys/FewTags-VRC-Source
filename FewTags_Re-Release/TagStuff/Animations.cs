            ...
            foreach (var part in parts)
            {
                ...
                else
                {
                    for (int i = 0; i < part.Text.Length; i++)
                    {
                        ...
                        if (shouldGlitch && UnityEngine.Random.Range(0f, 1f) < 0.25f)
                        {
                            string glitchChars = "0123456789ABCDEF!@#$%^&*()_+-=[]{}|;':\",./<>?~`";
                            currentChar = glitchChars[UnityEngine.Random.Range(0, glitchChars.Length)];
                        }
                        ...
                        else if (shouldGlitch && UnityEngine.Random.Range(0f, 1f) < 0.4f)
                        {
                            Color glitchColor = new Color(
                        ...
                        if (shouldRotate || (shouldGlitch && UnityEngine.Random.Range(0f, 1f) < 0.3f))
                        {
                            float rotation = UnityEngine.Random.Range(-45f, 45f);
                            sb.Append($"<rotate={rotation:F1}>");
                            hasRotationTag = true;
                        }

                        if (shouldSize || (shouldGlitch && UnityEngine.Random.Range(0f, 1f) < 0.25f))
                        {
                        ...
                        if (shouldFlicker || (shouldGlitch && UnityEngine.Random.Range(0f, 1f) < 0.35f))
                        {
                            float alpha = UnityEngine.Random.Range(0.2f, 1f);
                        ...
                        if (shouldBold || (shouldGlitch && UnityEngine.Random.Range(0f, 1f) < 0.2f))
                        {
                            sb.Append("<b>");
                            hasBoldTag = true;
                        }
